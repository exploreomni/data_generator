...
class Table(type):
    ...
    def __init__(cls, name, bases, attrs, **kw):
        ...
        cls.instances = (
            list()
        )  # stores the instances of this class which have been generated
        cls.__seed_defaults__ = (
            dict()
        )  # stores the default_factory for fields using itertools.count (smoth sequences)
                        ...
                        if hasattr(v, "default_factory"):
                            if hasattr(v.default_factory, "__qualname__"):
                                if v.default_factory.__qualname__ == "count.__next__":
                                    new_seed = (
                                        max(
                                            cls.instances, key=attrgetter(k)
                                        ).__getattribute__(k)
                                        + 1
                                    )
                                    set_(
                                        cls.__seed_defaults__,
                                        [cls, k],
                                        itertools.count(new_seed).__next__,
                                    )
    ...
    def unique(cls, field_name, func, *args, **kwargs) -> Any:
        ...
        if attempt_count > MAX_UNIQUE_ATTEMPTS:
            raise Exception(
                """Difficult to generate unique value... """
        ...
        if hash_key:
            ...
            if type(retval) == dict:
                raise Exception(
                    f"for dictionary based generators such as {func.__name__} "
    ...
    def pick_existing_object(cls, filter_func=None) -> Any:
        # pick at random from the subset that match the filter_func
        if filter_func:
            filtered_instances = list(filter(filter_func, cls.instances))
            index = random.randint(0, len(filtered_instances) - 1)
            return filtered_instances[index]
    ...
    def __init__snowflake(cls):
        if not cls.__snowflake__:
            ...
            cls.__snowflake_engine__ = create_engine(
                URL(
    ...
    def push_to_sf(cls) -> None:
        ...
        the_table = SA.Table(
            cls.__table_name__,
            SA.MetaData(schema=os.environ["SF_SCHEMA"]),
        ...
        print(
            f"Loaded {result[0][3]} rows to Snowflake {os.environ['SF_DATABASE']}"
